...
export class Page2 extends BasePage {
  ...
  async start(): Promise<boolean> {
    ...
    this.tasks.set("Expand and Refresh Connection", () =>
    ...
    let formBuilder = this.view.modelBuilder
      .formContainer()
      .withFormItems([optionsComponent], layout);

    if (this.optionsDropdown) {
      this.wizard.doneButton.onClick(() =>
        this.tasks.get(String(this.optionsDropdown?.value)).call()
      );
    }
  ...
  private async createOptionsDropdown(): Promise<azdata.FormComponent> {
    let optionsDropdown = this.view.modelBuilder
      .dropDown()
      .withProperties({
        required: true,
  ...
  private newQuery() {
    let sqlContent =
      "SELECT * FROM " + this.model.database + ".INFORMATION_SCHEMA.TABLES";
    vscode.workspace
      .openTextDocument({ language: "sql", content: sqlContent })
      .then((doc) => {
  ...
  private refreshConnection() {
    azdata.objectexplorer
      .getNode(this.model.server.connectionId)
      .then((node) => {
        if (node) {
          node.refresh();
  ...
  private viewConnectionString() {
    azdata.connection
      .getConnectionString(this.model.server.connectionId, true)
      .then(
        (connection) => {
          if (connection) {
